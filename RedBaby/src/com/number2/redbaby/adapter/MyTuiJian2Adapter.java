...
public class MyTuiJian2Adapter extends BaseAdapter {

	Context context;
	List<Brand> brandList;

	public MyTuiJian2Adapter(Context context, List<Brand> brandList) {
	...
	@Override
	public int getCount() {
		return brandList.get(1).value.size();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		Picasso.with(context).load(brandList.get(1).value.get(position).pic).into(viewHolder.tuijian_iv);
		viewHolder.tuijian_tv.setText(brandList.get(1).value.get(position).name);
		return convertView;
	}
